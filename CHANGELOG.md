...
- fix: `keyfactor_security_role` `Read` never actually contacted Keyfactor Command — the `GetSecurityRole` call had been commented out since the resource was first written, so `Read` just re-set whatever was already in Terraform state. `Read` now calls `GetSecurityRole` and rebuilds state from the server response, so out-of-band name/description/permission drift is now detected instead of echoing stale state
- fix: `keyfactor_security_role` `Update` could still clear a role's permissions on a completely unrelated update even with the `Optional`+`Computed` fix above — confirmed live against a real Command instance, Command's `PUT /Security/Roles` is a full-replace endpoint, and omitting `Permissions` from the request body entirely (not just sending an explicit `null`) still clears it server-side. `buildSecurityRoleUpdateArg` now resends the role's current state permissions explicitly whenever the config omits the attribute, instead of omitting the field
...
- test(demo): added real-`terraform`-CLI lab lifecycle validation infrastructure — `terraform/security_identity_demo/`, `terraform/security_role_permissions_demo/`, `terraform/security_role_demo/`, `terraform/pam_provider_type_demo/`, `terraform/template_role_binding_demo/`, and `terraform/certificate_authority_demo/` directories with matching top-level `GNUmakefile` targets, following the existing `terraform/oauth_security_demo/` convention, used to validate several of the fixes above against a real Command lab via the actual `terraform` binary rather than only Go-level `resource.UnitTest`.

## Pending (before GA v2.9.1)

- **GA dependency gate:** `go.mod` is pinned to `keyfactor-go-client/v3 v3.5.6-rc.2`. Keyfactor/keyfactor-go-client#55 and #56 have since merged into `v3`, but no GA tag has been cut yet; the gate now also includes Keyfactor/keyfactor-go-client#57 (fixing the `TemplatePolicy` full-replace gap described below), which should land in the same `v3.5.6` GA cut rather than requiring a separate patch release immediately after. Move the pin (and the dependency note above) to GA `v3.5.6` once all three are released. This release stays a prerelease until then.
...
- **Known open gap (not yet fixed):** `keyfactor_template_role_binding` attach/detach fails against any template linked to an enrollment pattern with `'Policies' cannot be empty` — Command's `PUT /Templates` full-replace endpoint clears a template's `TemplatePolicy` object when it's omitted from the request, and neither `keyfactor-go-client/v3`'s `UpdateTemplateArg`/`GetTemplateResponse` nor the provider modeled this field at all. Root cause confirmed live against Command 25.4.1; tracked as [GitHub issue #180](https://github.com/keyfactor-pub/terraform-provider-keyfactor/issues/180). Fix is staged but NOT yet landed on this branch — a go-client fix is open as [Keyfactor/keyfactor-go-client#57](https://github.com/Keyfactor/keyfactor-go-client/pull/57) (unmerged), and the provider-side fix depends on that landing and being published (RC or GA) before it can be picked up here.
