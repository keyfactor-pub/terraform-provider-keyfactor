...
- fix: `keyfactor_certificate_template` `Update` no longer silently re-grants a role that `keyfactor_template_role_binding` had already revoked earlier in the same apply. `allowed_requesters`/`use_allowed_requesters` are Optional+Computed and can be pinned by the plan to a KNOWN but STALE prior-state value (absorbed by a previous `Read` from a binding-managed role the template's own config never declares) whenever any other template attribute changes in the same apply — the #195/#190 read-modify-write preservation was keying its "trust the fresh GET over plan" decision on whether that plan value was null, which can't tell "config declared this" apart from "plan was pinned to a stale value" for a Computed attribute. If the same apply also destroys the `keyfactor_template_role_binding` managing that role, the binding's `Delete` removes the role server-side before the template's `Update` runs, but `Update` still saw a non-null pinned plan and skipped the fresh GET, re-`PUT`ting the stale, just-revoked role list with `UseAllowedRequesters=true` — with no diff and no error, and the next `Read` re-absorbed the re-granted role right back into state, making the revocation permanently ineffective. The preservation decision for these two fields now keys on whether CONFIG (not plan) declares them, so the fresh GET always wins when they're undeclared. Trade-off: in that same narrow interleaving (binding destroyed + another template attribute changed, in one apply), the fresh GET's value can legitimately disagree with the stale value Terraform core already recorded during planning, surfacing a one-time "Provider produced inconsistent result after apply" error that self-corrects on the next apply — a loud, recoverable failure in exchange for eliminating a silent, permanent privilege re-grant, since no plan-time fix is possible (the binding's `Delete` that invalidates the pinned plan value runs after planning). Separately, the fresh-GET-before-update fetch that this preservation relies on is no longer conditionally skipped when every writable field looked declared: that skip condition had already drifted out of sync with the fields it was meant to mirror (it didn't account for an otherwise-fully-declared `template_policy` block missing just one nested field), which could silently clear that one nested field — the same #195 bug class again. The fetch is now unconditional (one extra inexpensive `GET` per update in the fully-declared case).
...
- fix: switching an existing `keyfactor_certificate_authority` between auth variants (OAuth and client-certificate) in a single apply no longer fails with "Provider produced inconsistent result after apply". `token_url`/`client_id`/`scope`/`audience` and the read-only `auth_certificate_issued_dn`/`auth_certificate_issuer_dn`/`auth_certificate_thumbprint` attributes previously used a bare "carry forward prior state when undeclared" plan modifier, the same gap the schedule-pair fix above addresses for `full_scan`/`incremental_scan`/`threshold_check`: it had no way to know the OTHER auth variant was taking over and pinned the plan to the stale, now-cleared value from the outgoing variant, while the fix above (deriving which variant is in use and omitting the other) correctly cleared it server-side. These attributes now use the same sibling-aware plan modifier pattern, nulling an attribute at plan time whenever the other auth variant is declared in config, so switching variants in one apply plans and applies cleanly. A follow-up fix addressed the remaining gap on the metadata side: the three `auth_certificate_issued_dn`/`_issuer_dn`/`_thumbprint` attributes' plan modifier only ever nulled them when an OAuth attribute became declared (the outgoing-cert-auth direction) — it had no trigger at all for the OPPOSITE direction, so switching an OAuth CA TO client-certificate auth, or rotating `auth_certificate` on an already cert-auth CA, still pinned stale (null, for a switch; the old certificate's, for a rotation) metadata onto the plan while Command's response carried the real, freshly computed values for the new certificate. The plan modifier now also watches `auth_certificate` itself: when its config value differs from what was previously in state (a genuine incoming switch or rotation), the three metadata attributes' plan now stays `(known after apply)` instead of being resurrected or nulled, so the server-computed value is never compared against a value the provider guessed; an unchanged, steadily-redeclared `auth_certificate` (the normal shape of every apply for this write-only field) still carries the prior metadata forward with no diff.
