...
- fix: `keyfactor_certificate_authority` now rejects invalid attribute combinations at plan time instead of silently accepting them: `enforce_unique_dn` and `new_end_entity_on_renew_and_reissue` can no longer both be set to `true`; `new_end_entity_on_renew_and_reissue = false` is rejected for HTTPS CAs (`ca_type = 1`); and `allowed_enrollment_types`/`use_allowed_requesters`/`allowed_requesters` are rejected when `standalone` is explicitly set to `false` **and** set to a genuinely conflicting value (a non-zero `allowed_enrollment_types`, `use_allowed_requesters = true`, or a non-empty `allowed_requesters`) — an explicit no-op value for any of these three (`0`, `false`, `[]`) alongside `standalone = false` is allowed, since Command itself accepts it as a no-op and the provider's own import-then-codify workflow (copying `terraform state show` output into config) routinely produces exactly that shape for an existing, previously-working non-standalone CA. Leaving any of these attributes unset (or referencing an unresolved value) is never an error
