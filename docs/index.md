...
subcategory: ""
description: |-
  Terraform provider for managing Keyfactor Command resources: certificates, certificate stores, templates, security roles and identities, and more.
---
...
- `request_timeout` (Number) Global timeout, in seconds, for HTTP requests to Keyfactor Command instance. This value is enforced as the HTTP client timeout on every API request the provider makes, not just a subset. A value that is too small can cause otherwise-valid, long-running operations (e.g. PFX enrollment) to fail with a timeout error. Values above 3600 are clamped to 3600. This can also be set via the `KEYFACTOR_CLIENT_TIMEOUT` environment variable.Default value is `60`.
