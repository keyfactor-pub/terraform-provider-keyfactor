...
- `full_scan_daily_time` (String) UTC time-of-day, formatted "HH:MM:SS" (e.g. "07:00:00"), that sets a once-daily full synchronization schedule for this certificate authority. Mutually exclusive with full_scan_interval_minutes. Command only preserves the time-of-day component and rewrites the date anchor server-side, so only "HH:MM:SS" is accepted -- a full timestamp can never round-trip.
...
- `threshold_check_daily_time` (String) UTC time-of-day, formatted "HH:MM:SS" (e.g. "07:00:00"), that sets a once-daily threshold monitoring check schedule on this CA. Mutually exclusive with threshold_check_interval_minutes. Command only preserves the time-of-day component and rewrites the date anchor server-side, so only "HH:MM:SS" is accepted -- a full timestamp can never round-trip.
