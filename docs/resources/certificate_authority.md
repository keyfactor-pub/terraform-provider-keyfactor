...
- `full_scan_daily_time` (String) UTC time-of-day, formatted "HH:MM:SS" (e.g. "07:00:00"), that sets a once-daily full synchronization schedule for this certificate authority. Mutually exclusive with full_scan_interval_minutes. Omit to leave the server-side schedule unmanaged (preserved on update); set 0 (interval) or "" (daily) explicitly to clear it.
...
- `threshold_check_daily_time` (String) UTC time-of-day, formatted "HH:MM:SS" (e.g. "07:00:00"), that sets a once-daily threshold monitoring check schedule on this CA. Mutually exclusive with threshold_check_interval_minutes. Omit to leave the server-side schedule unmanaged (preserved on update); set 0 (interval) or "" (daily) explicitly to clear it.
