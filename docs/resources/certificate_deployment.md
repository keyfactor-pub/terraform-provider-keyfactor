...
- `max_inventory_wait` (Number) Maximum seconds to wait for inventory confirmation after deployment. Null (default) waits indefinitely (current behavior). 0 skips inventory verification entirely (fire-and-forget). Values greater than 0 wait up to that many seconds, then proceed with a warning if not confirmed.
