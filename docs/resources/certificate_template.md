...
- `allowed_enrollment_types` (Number) Bitmask of allowed enrollment types: 0=none (disabled), 1=PFX, 2=CSR, 3=both. Setting to 0 effectively disables the template.
- `allowed_requesters` (List of String) List of security roles allowed to enroll.
- `enrollment_fields` (Attributes List) Custom enrollment fields for CSR/PFX enrollment. (see [below for nested schema](#nestedatt--enrollment_fields))
...
- `template_defaults` (Attributes List) Default values for subject fields. (see [below for nested schema](#nestedatt--template_defaults))
...
- `template_regexes` (Attributes List) Subject field regex validation rules. (see [below for nested schema](#nestedatt--template_regexes))
- `use_allowed_requesters` (Boolean) Whether to restrict enrollment to specific requesters.

### Read-Only

- `certificate_cleanup_enabled` (Boolean) Whether certificate cleanup is enabled (v25+, read-only).
...
- `delete_with_archived_key` (Boolean) Whether to delete certificates with archived keys (v25+, read-only).
...
- `manageability` (Number) Manageability level (v25+, read-only).
- `oid` (String) OID of the template. Read-only.
- `template_name` (String) Display name of the template as defined in the CA. Read-only.
- `time_after_expiration` (Number) Time after expiration value (v25+, read-only).
- `time_after_expiration_units` (String) Units for time_after_expiration (v25+, read-only).
