...
- `force_template_default` (Boolean) Write-only directive: when true, forces this pattern to become the template's default even if another pattern currently holds that status. Not persisted -- must be re-declared on every apply where it is needed; always reads back as null.
