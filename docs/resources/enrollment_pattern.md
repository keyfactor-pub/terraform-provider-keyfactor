...
- `force_template_default` (Boolean) Write-only directive: when true, forces this pattern to become the template's default even if another pattern currently holds that status. Not persisted server-side. To un-set the directive, leave this attribute undeclared on a later apply.
