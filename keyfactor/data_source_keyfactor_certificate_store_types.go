...
func certStoreTypeToAttrValue(st api.CertificateStoreType) attr.Value {
	...
	return types.Object{
		...
		Attrs: map[string]attr.Value{
			...
			"local_store":              types.Bool{Value: st.LocalStore},
			...
			"server_required":          types.Bool{Value: st.ServerRequired},
			"power_shell":              types.Bool{Value: st.PowerShell},
			"blueprint_allowed":        types.Bool{Value: st.BlueprintAllowed},
