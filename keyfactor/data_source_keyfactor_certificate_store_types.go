...
func certStoreTypeToAttrValue(st api.CertificateStoreType) attr.Value {
	...
	return types.Object{
		...
		Attrs: map[string]attr.Value{
			...
			"local_store":              boolPtrToTfBool(st.LocalStore),
			...
			"server_required":          boolPtrToTfBool(st.ServerRequired),
			"power_shell":              boolPtrToTfBool(st.PowerShell),
			"blueprint_allowed":        boolPtrToTfBool(st.BlueprintAllowed),
