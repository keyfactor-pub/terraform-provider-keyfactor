...
// TestUnitCAReadPreservesDailyScheduleAndLeavesOthersUnset is a regression
// test for the bug where caResponseToState only recognized the Interval
// variant of a Command schedule. A CA whose FullScan/IncrementalScan/
// ThresholdCheck is Daily-shaped (Command's KeyfactorSchedule model supports
// Interval, Daily, Weekly, Monthly, ExactlyOnce, and Immediate as mutually
// exclusive variants) fell into the same "no schedule at all" Null branch as
// an actually-unconfigured schedule, because there was no schema attribute to
// hold the Daily value. Verified live against a real CA
// (int25-4-1.kftestlab.com, CA id 1) whose real FullScan.Daily.Time schedule
// was silently wiped by an Update that never touched schedule config.
//
// This is the "unrelated schedules stay untouched" companion to
// TestUnitCAReadPreservesDailySchedule in
// resource_keyfactor_certificate_authority_schedule_unit_test.go, which covers
// the same root bug with a focus on the Interval/Daily distinction; kept as a
// separate test (renamed from the same original name to avoid a
// redeclaration) since it additionally asserts IncrementalScan/ThresholdCheck
// remain Null when genuinely unconfigured.
func TestUnitCAReadPreservesDailyScheduleAndLeavesOthersUnset(t *testing.T) {
...
// NOTE: this file used to continue with TestUnitCANormalizePropertiesJSON,
// TestUnitCAPropertiesModifierSuppressesReorderedJSONDiff,
// TestUnitCAPropertiesModifierSurfacesRealChange,
// TestUnitCABarePropertiesUseStateForUnknownDoesNotSuppressReorderedJSONDiff,
// TestUnitCAPropertiesModifierUndeclaredCopiesState,
// TestUnitCAPropertiesModifierCreateStaysUnknown,
// TestUnitCAValidateConfigRejectsEnforceUniqueDNWithNewEndEntity,
// TestUnitCAValidateConfigAllowsEnforceUniqueDNOrNewEndEntityAlone,
// TestUnitCAValidateConfigRejectsNewEndEntityFalseForHTTPSCa,
// TestUnitCAValidateConfigAllowsNewEndEntityUnsetOrUnknownForHTTPSCa,
// TestUnitCAValidateConfigAllowsStandaloneOnlyAttributesWhenStandaloneUnsetOrUnknown,
// TestUnitCAValidateConfigAllowsStandaloneOnlyAttributesWhenStandaloneTrue, and
// TestUnitCAReadAuthCertificateMetadataNullWhenAbsent -- all byte-for-byte
// identical (verified via diff) to tests already in
// resource_keyfactor_certificate_authority_optional_computed_unit_test.go,
// except for TestUnitCAPropertiesModifierSuppressesReorderedJSONDiff and
// TestUnitCAPropertiesModifierUndeclaredCopiesState, whose copies here
// predated the AttributeConfig nil-check added to
// normalizedJSONPropertiesModifier.Modify and so omitted AttributeConfig
// from the constructed tfsdk.ModifyAttributePlanRequest -- the
// optional_computed_unit_test.go versions are current. Removed here to
// avoid a duplicate-symbol compile error; no coverage was lost.
